...
export default function KitchenGalleryPage() {
  ...
  const handleLengthChange = (val: number) => {
    ...
    if (activeModalDesign) {
      setCalcResult(estimateIndiaKitchenCost({
        lengthFt: val,
        widthFt: w,
        areaSqFt: sqft,
  ...
  const handleTierChange = (val: 'Economy' | 'Standard' | 'Premium' | 'Ultra Luxury') => {
    setCalcTier(val);
    if (activeModalDesign) {
      setCalcResult(estimateIndiaKitchenCost({
        lengthFt: calcLength,
  ...
  useEffect(() => {
    ...
    progressTimer = setInterval(() => {
      setLoadingProgress((prev) => {
        ...
        if (prev < 78) {
          setLoadingStage('Loading modular specs & IS:710 materials...');
  ...
  const handleOpenModal = useCallback(async (design: KitchenDesign) => {
    ...
    const l = dimMatch ? Math.round(Number(dimMatch[1])) : 12;
    const w = dimMatch ? Math.round(Number(dimMatch[2])) : 10;
    const s = sqftMatch ? Math.round(Number(sqftMatch[1])) : (l > 0 && w > 0 ? l * w : 120);
    const tier = (design.quality_tier as any) || 'Standard';
    ...
    setCalcResult(estimateIndiaKitchenCost({
      ...
      widthFt: w,
      areaSqFt: s,
      layoutShape: (design.layout_shape as KitchenLayoutShape) || 'L-Shape',
      qualityTier: tier
    }));
  ...
  const handleOpenCalculator = (design: KitchenDesign) => {
    const params = new URLSearchParams({
      ...
      shape: design.layout_shape || 'L-Shape',
      title: design.title || '',
      tier: calcTier,
    });
          ...
          <div className="space-y-8 py-4">
            ...
            <div className="max-w-md mx-auto p-5 bg-white dark:bg-zinc-900 rounded-3xl border border-gray-200 dark:border-zinc-800 shadow-lg text-center space-y-3.5">
              ...
              <p className="text-[11px] text-gray-400 dark:text-zinc-500 font-medium">
                Fetching HD 9:16 modular layouts &amp; live Indian rates...
              </p>
            </div>
              ...
              {[1, 2, 3, 4].map((n) => (
                ...
                >
                  <div className="relative aspect-[9/16] w-full bg-gray-100 dark:bg-zinc-800/80 flex flex-col items-center justify-center p-4">
                    <i className="fas fa-kitchen-set text-3xl text-gray-300 dark:text-zinc-700 mb-2"></i>
